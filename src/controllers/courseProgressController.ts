...
async function assertCourseAccess(userId: string, courseSlug: string) {
  const { bundle, courseIds } = await getEntitledCourseIds(userId);
  if (!bundle) {
    return { ok: false as const, status: 403, message: 'No active bundle on this account.' };
  }
  if (!courseIds.includes(courseSlug)) {
    return { ok: false as const, status: 403, message: 'This course is not included in your bundle.' };
  }
  return { ok: true as const, bundle };
}

export async function listMyCourses(req: AuthRequest, res: Response) {
  if (!req.user) {
...
export async function updateCourseProgress(req: AuthRequest, res: Response) {
  if (!req.user) {
    return res.status(401).json({ message: 'Authentication required.' });
  }
