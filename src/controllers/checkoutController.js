...
import { getRazorpay, paymentProvider, rupeesToPaise, verifyRazorpaySignature, } from '../utils/razorpay.js';
import { makeOrderId, makePaymentId, publicUser, uniqueAffiliateCode, } from '../utils/userPublic.js';
...
async function findStudentByAffiliateCode(code) {
    if (!code)
        return null;
...
function readCheckoutFields(body) {
    return {
...
async function assertCheckoutCredentials(fields, authUserId) {
...
async function resolveBuyer(fields, authUserId) {
    ...
    let sponsorCode = user.sponsorCode ? normalizeReferral(user.sponsorCode) || null : null;
    if (fields.referral) {
...
async function fulfillEnrollment(opts) {
    ...
    if (order) {
        ...
        if (opts.razorpayPaymentId)
            order.razorpayPaymentId = opts.razorpayPaymentId;
        if (opts.razorpaySignature)
            order.razorpaySignature = opts.razorpaySignature;
        await order.save();
    }
    else {
...
export async function completeCheckout(req, res) {
    ...
    const method = PAYMENT_METHODS.includes(fields.paymentMethod)
...
export async function verifyCheckoutPayment(req, res) {
    ...
    if (order.status === 'paid') {
        ...
        if (!user)
            return res.status(404).json({ message: 'User not found for paid order.' });
        try {
            await distributeCommissionsForPaidOrder({
                orderId: order.orderId,
                amount: order.amount,
...
export async function failCheckoutOrder(req, res) {
...
export async function razorpayWebhook(req, res) {
    ...
    if (event === 'payment.failed' && order.status !== 'paid') {
        ...
        if (paymentEntity?.id)
            order.razorpayPaymentId = String(paymentEntity.id);
        await order.save();
    }
...
export async function listAdminOrders(_req, res) {
