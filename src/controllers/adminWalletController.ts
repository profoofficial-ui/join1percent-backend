...
export async function listAdminWalletTransactions(req: AuthRequest, res: Response) {
  const {
    userId,
    search,
    type,
  ...
  const txnQuery: Record<string, unknown> = {};
  ...
  const creditTxns = txnDocs.map((doc) => {
    ...
    return {
      ...
      type: doc.type,
      category: (doc.category || (doc.level && doc.level > 0 ? 'commission' : 'adjustment')) as string,
      status: (doc.status as 'Credit' | 'Debit') || 'Credit',
      amount: doc.amount,
      level: doc.level || 0,
  ...
  const payoutTxns = payoutDocs.map((doc) => {
    ...
    let statusDirection: 'Credit' | 'Debit' = 'Debit';
    if (isPaid) {
...
export async function adjustAdminWallet(req: AuthRequest, res: Response) {
  const { userId, amount, type, reason } = req.body as {
    userId: string;
    amount: number;
    type: 'Credit' | 'Debit';
    reason?: string;
