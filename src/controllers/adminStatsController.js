...
async function sumCreditedCommissions(orderIds) {
    if (!orderIds.length)
        return 0;
...
function estimateAffiliateShare(orders, universalLevel1, bundleLevel1) {
    return orders.reduce((sum, order) => {
        if (!order.sponsorCode)
            return sum;
        const level1 = bundleLevel1.has(order.bundleSlug)
            ? bundleLevel1.get(order.bundleSlug)
            : universalLevel1;
...
export async function getAdminStats(_req, res) {
    ...
    const bundleCourseCount = new Map(bundles.map((bundle) => [bundle.slug, (bundle.courseIds || []).length]));
    const bundleLevel1 = new Map();
    ...
    const courseSeats = orders.reduce((sum, order) => sum + (bundleCourseCount.get(order.bundleSlug) || 0), 0);
    const creditedAffiliateShare = await sumCreditedCommissions(orders.map((o) => o.orderId));
    const estimatedAffiliateShare = creditedAffiliateShare > 0
    ...
    return res.json({
        ...
        kpis: {
            ...
            netAfterEstimate,
            commissionNote: creditedAffiliateShare > 0
...
export async function getCommissionSettings(_req, res) {
...
export async function updateCommissionSettings(req, res) {
    ...
    const doc = await CommissionSettings.findOneAndUpdate({ key: 'default' }, { levels, percent }, { upsert: true, new: true, setDefaultsOnInsert: true });
    return res.json({
        commission: {
            levels: doc?.levels ?? levels,
            percent: doc?.percent?.slice(0, 3) ?? percent,
