...
async function findCourseByParam(idOrSlug) {
...
export async function createCourse(req, res) {
    ...
    const level = COURSE_LEVELS.includes(levelRaw)
        ? levelRaw
        : 'Beginner';
...
export async function updateCourse(req, res) {
    ...
    const description = req.body?.description !== undefined ? String(req.body.description).trim() : course.description;
    if (!title || !description) {
        return res.status(400).json({ message: 'Title and description are required.' });
    }
    course.title = title;
    ...
    if (req.body?.tagline !== undefined)
        course.tagline = String(req.body.tagline).trim();
    if (req.body?.image !== undefined)
        course.image = String(req.body.image).trim();
    if (req.body?.instructor !== undefined)
        course.instructor = String(req.body.instructor).trim();
    if (req.body?.level !== undefined) {
        const levelRaw = String(req.body.level);
        if (COURSE_LEVELS.includes(levelRaw)) {
            course.level = levelRaw;
        }
    }
    if (req.body?.duration !== undefined)
        course.duration = String(req.body.duration).trim();
    if (req.body?.language !== undefined)
        course.language = String(req.body.language).trim();
    if (req.body?.category !== undefined)
        course.category = String(req.body.category).trim();
    if (req.body?.videoUrl !== undefined)
        course.videoUrl = String(req.body.videoUrl).trim();
    if (req.body?.outcomes !== undefined)
        course.outcomes = asStringList(req.body.outcomes);
    if (req.body?.requirements !== undefined)
        course.requirements = asStringList(req.body.requirements);
    if (req.body?.tags !== undefined)
        course.tags = asStringList(req.body.tags);
