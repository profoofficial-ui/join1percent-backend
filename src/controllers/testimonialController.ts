...
export async function listAdminTestimonials(_req: AuthRequest, res: Response) {
  try {
...
export async function updateTestimonial(req: AuthRequest, res: Response) {
  try {
    ...
    if (company !== undefined) testimonial.company = String(company).trim();
    ...
    if (careerImpact !== undefined) testimonial.careerImpact = String(careerImpact).trim();
    if (orderIndex !== undefined) testimonial.orderIndex = Number(orderIndex) || 0;
