...
function normalizeCommissionPercent(raw) {
...
async function findBundleByParam(slugOrId) {
...
async function assertCourseIds(courseIds) {
    if (courseIds.length === 0) {
        return { ok: false, message: 'At least one course is required.' };
    }
    ...
    if (missing.length) {
        return { ok: false, message: `Unknown course ids: ${missing.join(', ')}` };
    }
    return { ok: true };
}
...
export async function createBundle(req, res) {
    ...
    const courseIds = Array.isArray(req.body?.courseIds)
        ? req.body.courseIds.map((id) => String(id).trim()).filter(Boolean)
        : [];
    if (!name) {
...
export async function updateBundle(req, res) {
    ...
    if (req.body?.image !== undefined)
        bundle.image = String(req.body.image).trim();
    if (req.body?.tagline !== undefined)
        bundle.tagline = String(req.body.tagline).trim();
