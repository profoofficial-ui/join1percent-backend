...
import { PayoutRequest, PAYOUT_STATUSES, type PayoutStatus } from '../models/PayoutRequest.js';
...
export async function listAdminPayouts(req: AuthRequest, res: Response) {
  if (!req.user) {
    return res.status(401).json({ message: 'Authentication required.' });
  }
  ...
  const filter =
    status === 'all' ? {} : { status: PAYOUT_STATUSES.includes(status as PayoutStatus) ? status : 'pending' };
...
export async function reviewPayout(req: AuthRequest, res: Response) {
  if (!req.user) {
    return res.status(401).json({ message: 'Authentication required.' });
  }

  const id = String(req.params.id || '').trim();
  const nextStatus = String(req.body?.status || '').trim() as PayoutStatus;
  ...
  payout.reviewedBy = req.user.id as unknown as typeof payout.reviewedBy;
