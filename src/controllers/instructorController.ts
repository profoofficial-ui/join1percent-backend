...
export async function listPublicInstructors(_req: Request, res: Response) {
  try {
...
export async function listAdminInstructors(_req: AuthRequest, res: Response) {
  try {
...
export async function createInstructor(req: AuthRequest, res: Response) {
  try {
...
export async function updateInstructor(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
    const { name, role, company, bio, avatarUrl, studentsCount, coursesCount, rating, orderIndex, isActive } = req.body || {};
    ...
    if (role !== undefined) instructor.role = String(role).trim();
    if (company !== undefined) instructor.company = String(company).trim();
    if (bio !== undefined) instructor.bio = String(bio).trim();
    if (avatarUrl !== undefined) instructor.avatarUrl = String(avatarUrl).trim();
    if (studentsCount !== undefined) instructor.studentsCount = Number(studentsCount) || 0;
    if (coursesCount !== undefined) instructor.coursesCount = Number(coursesCount) || 0;
    if (rating !== undefined) instructor.rating = Number(rating) || 5;
    if (orderIndex !== undefined) instructor.orderIndex = Number(orderIndex) || 0;
    if (isActive !== undefined) instructor.isActive = Boolean(isActive);
...
export async function deleteInstructor(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
    const instructor = await Instructor.findById(id);
    if (!instructor) {
