...
export async function listAdminUsers(req: AuthRequest, res: Response) {
  try {
    const { q, plan, role, status } = req.query as {
      q?: string;
      plan?: string;
      role?: string;
      status?: string;
    ...
    const orderMap = new Map<string, { count: number; totalSpent: number }>();
    for (const stat of orderStats) {
      if (stat._id) orderMap.set(stat._id.toLowerCase(), { count: stat.count, totalSpent: stat.totalSpent });
    }

    const referralMap = new Map<string, number>();
    for (const stat of referralStats) {
      if (stat._id) referralMap.set(stat._id.toUpperCase(), stat.count);
    }
...
export async function updateAdminUser(req: AuthRequest, res: Response) {
  try {
    ...
    if (role !== undefined && ['admin', 'student'].includes(role)) user.role = role as any;
    if (planSlug !== undefined) user.planSlug = planSlug || null;
    if (walletBalance !== undefined && !Number.isNaN(Number(walletBalance))) {
      user.walletBalance = Math.max(0, Number(walletBalance));
    }
    if (isActive !== undefined) user.isActive = Boolean(isActive);
    if (sponsorCode !== undefined) user.sponsorCode = sponsorCode ? String(sponsorCode).trim().toUpperCase() : null;
