...
export async function listAdminKyc(req: AuthRequest, res: Response) {
  const status = String(req.query?.status || 'all').trim();
  const filter =
    status !== 'all' && (KYC_STATUSES as readonly string[]).includes(status)
      ? { status }
      : {};
...
export async function reviewKyc(req: AuthRequest, res: Response) {
  ...
  doc.reviewedBy = req.user.id as unknown as typeof doc.reviewedBy;
  await doc.save();
