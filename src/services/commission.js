import { CommissionSettings, DEFAULT_COMMISSION, } from '../models/CommissionSettings.js';
import { Bundle } from '../models/Bundle.js';
import { Order } from '../models/Order.js';
import { User } from '../models/User.js';
import { WalletTxn } from '../models/WalletTxn.js';
export async function getOrCreateCommission() {
...
export async function resolveCommissionRates(bundleSlug) {
...
export async function distributeCommissionsForPaidOrder(opts) {
    ...
    if (!normalizeCode(opts.sponsorCode) && normalizeCode(opts.buyer.sponsorCode)) {
        await Order.updateOne({ orderId: opts.orderId }, { $set: { sponsorCode: normalizeCode(opts.buyer.sponsorCode) } });
    }
    let credited = 0;
    ...
    for (let level = 1; level <= rates.levels; level += 1) {
        ...
        if (String(affiliate._id) === buyerId)
            break;
        ...
        if (!alreadyLevel && amount > 0) {
            try {
                ...
                await User.updateOne({ _id: affiliate._id }, { $inc: { walletBalance: amount } });
                credited += 1;
                console.log(`[commission] L${level} ₹${amount} (${percent}%) → ${affiliate.affiliateCode} for order ${opts.orderId}`);
            }
            catch (error) {
...
export async function repairSponsorCodesFromOrders() {
    ...
    for (const order of orders) {
        ...
        if (!sponsorCode)
            continue;
        if (normalizeCode(buyer.affiliateCode) === sponsorCode)
            continue;
...
export async function backfillMissingCommissions() {
    ...
    for (const order of orders) {
        ...
        if (result.credited > 0 || after > before)
            fixed += 1;
    }
    return fixed;
}
